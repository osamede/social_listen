...
public class CreateAccountController extends AdminController {
	...
	public String create(@RequestParam("alias") String alias, 
		...
		if (user1email != "") {
			...
			else if (user1role.equals("Normal"))
		...
		if (user2email != "") {
			...
			if(user2role.equals("Admin"))
				user.setRole(Role.ROLE_ADMIN);
			else if (user2role.equals("Normal"))
				user.setRole(Role.ROLE_USER);			
			getAdminService().saveUser(user);
		}
		if (user3email != "") {
			...
			if(user3role.equals("Admin"))
				user.setRole(Role.ROLE_ADMIN);
			else if (user3role.equals("Normal"))
				user.setRole(Role.ROLE_USER);
			getAdminService().saveUser(user);
		}
		...
		salesMail.setSubject("Open Applicant Account Creation Process Completed for "+token.getCompanyName());
		salesMail.setFrom("Open Applicant Account Creation <openapplicant-account@openapplicant.org>");
		...
		getAdminService().sendAccountCreationEmail(salesMail);
	...
	public String processForm(
		...
		message.setTo(email);
		message.setFrom("Open Applicant Account Creation <openapplicant-account@openapplicant.org>");
		message.setSubject("Open Applicant Account Creation Confirmation");
		String body = "To complete the Open Applicant Account Creation process, please visit the following URL:\n"
			+ "http://beta.openapplicant.org/openapplicant/admin/createAccount/create?token="+token.getEntityInfo().getBusinessGuid();
		message.setText(body);
		getAdminService().sendAccountCreationEmail(message);

		// TODO send email to sales staff
		SimpleMailMessage salesMail = new SimpleMailMessage();
		salesMail.setTo("mrw@originatelabs.com");
		salesMail.setSubject("Open Applicant Account Creation Process Started for "+token.getCompanyName());
		salesMail.setFrom("Open Applicant Account Creation <openapplicant-account@openapplicant.org>");
