...
public class ExamDAO extends DomainObjectDAO<Exam> implements IExamDAO {
	...
	public List<Exam> findLatestVersionsByCompanyId(long companyId) {
		return getHibernateTemplate().find(
				"from " + Exam.class.getName() + 
				" where company.id = ? and nextVersion is null",
				companyId
		);
	}
	
	@SuppressWarnings("unchecked")
	public List<Exam> findLatestActiveVersionsByCompanyId(long companyId) {
		return getHibernateTemplate().find(
				"from " + Exam.class.getName() + 
				" where company.id = ? and nextVersion is null and active=true", 
				companyId
		);
	}

	
	public Exam findLatestVersionByArtifactId(String artifactId) {
		return findUniqueResult(
				"from " + Exam.class.getName() + 
				" where artifactId = :artifactId and nextVersion is null", 
				new Params("artifactId", artifactId)
		);
	}
	
	public Exam findLatestVersionByCompanyAndName(Company company, String name) {
		Params parameters = new Params();
		parameters.add("companyId", company.getId());
		parameters.add("name", name);
		return findUniqueResult(
				"from " + Exam.class.getName() +
				" where company.id=? and name=? and nextVersion is null",
				parameters
			);
	}
